...
begin
  if not private.is_hrx_admin() then raise exception 'forbidden'; end if;
  if nullif(btrim(coalesce(p_name, '')), '') is null or (v_email is null and v_phone is null) then raise exception 'invalid_client'; end if;
  ...
  return v_id;
exception when unique_violation then raise exception 'duplicate_client';
end;
$$;
