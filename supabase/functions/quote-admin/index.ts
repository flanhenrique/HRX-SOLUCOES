...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    return json({ providers: providers ?? [], requests: (requests ?? []).map((item) => ({ ...item, interpretation: interpretationByRequest.get(item.id) ?? null, draft: draftByRequest.get(item.id) ?? null })) }, 200, headers)
  }
  ...
  const retentions: Required<Retentions> = {
    iss: Math.max(0, Number(body.retentions?.iss ?? 0)), irrf: Math.max(0, Number(body.retentions?.irrf ?? 0)),
    pis: Math.max(0, Number(body.retentions?.pis ?? 0)), cofins: Math.max(0, Number(body.retentions?.cofins ?? 0)),
    csll: Math.max(0, Number(body.retentions?.csll ?? 0)), inss: Math.max(0, Number(body.retentions?.inss ?? 0)),
  }
  ...
  const update = {
    complexity_multiplier: complexity, urgency_multiplier: urgency, pre_discount_amount: preDiscountAmount,
    discount_percent: body.discountPercent, discount_status: discountTone(body.discountPercent), discount_amount: discountAmount,
    final_amount: finalAmount, payment_provider: provider, installments, boleto_fee_per_installment: feePerInstallment,
    payment_fee_total: paymentFeeTotal, retentions, retention_total: retentionTotal, retention_pricing_mode: mode,
    retention_net_target: retentionNetTarget, retention_gross_up_suggestion: grossUpSuggestion, estimated_net: estimatedNet,
    fiscal_review_required: fiscalReviewRequired, fiscal_review_confirmed: fiscalReviewConfirmed,
    fiscal_review_confirmed_by: fiscalReviewConfirmed ? user.id : null, fiscal_review_confirmed_at: fiscalReviewConfirmed ? now : null,
    notes: body.notes ?? draft.notes, status: Number(draft.base_amount) > 0 ? 'awaiting_review' : 'needs_scope', updated_at: now,
  }
  ...
  await db.from('quote_audit_log').insert({ request_id: body.requestId, actor_user_id: user.id, event_type: 'draft_updated', event_data: { discount: body.discountPercent, provider, installments, retentionTotal, retentionPricingMode: mode } })
