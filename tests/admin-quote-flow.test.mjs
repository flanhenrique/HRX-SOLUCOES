...
test('mobile separa lista e editor e usa fluxo comercial por etapas', async () => {
  const [editor, css, routeCss] = await Promise.all([read('src/quotes/AdminQuotes.tsx'), read('src/quotes/quote-commercial.css'), read('src/quotes/admin-quotes-mobile.css')])
...
test('ações de finalizar e excluir orçamento permanecem acessíveis acima do menu flutuante', async () => {
  const [routeCss, brandFix, editor, index] = await Promise.all([read('src/quotes/admin-quotes-mobile.css'), read('public/hrx-brand-fix.css'), read('src/quotes/AdminQuotes.tsx'), read('index.html')])
  assert.match(routeCss, /quote-review-card>footer/)
  assert.match(routeCss, /position:sticky!important/)
  assert.match(routeCss, /bottom:var\(--hrx-dock-clearance\)!important/)
  assert.match(routeCss, /quote-review-card>footer \.quote-primary/)
  assert.match(routeCss, /min-height:50px/)
  assert.doesNotMatch(brandFix, /quote-review-card|quote-editor-footer/)
