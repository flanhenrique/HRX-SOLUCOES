...
test('fiscal hub allows manual IE and latest quote keeps the client synchronized', async () => {
  const [fiscal, migration, main] = await Promise.all([
    read('src/quotes/AdminFiscalHub.tsx'),
  ...
  assert.match(fiscal, /↻ Atualizar dados/)
  ...
  assert.match(main, /admin-fiscal-manual-state\.css/)
})
