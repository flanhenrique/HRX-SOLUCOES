...
|---|---|---|---|---|---|---|
| `admin_users` | Cadastro de administradores e roles | `user_id (UUID)` | `role_idx` | AAL2 / Self-admin | SELECT (Authenticated) | Protegido por allowlist |
| `clients` | Carteira de clientes cadastrados | `id (UUID)` | `document_idx`, `name_idx` | Restrictive Admin AAL2 | SELECT, INSERT, UPDATE | Auto-catalogação via RPC |
...
| `quote_drafts` | Rascunhos operacionais de propostas | `id (UUID)` | `request_id_idx`, `status_idx` | Admin AAL2 | SELECT (Admin) / Mutations via RPC | Proteção contra exclusão de versionados |
...
| `quote_versions` | Snapshots imutáveis de propostas geradas | `id (UUID)` | `request_id_idx`, `version_idx` | Admin AAL2 (Read-only) | SELECT (Admin) / INSERT via Edge Function | **Imutabilidade**: UPDATE e DELETE bloqueados |
...
| `quote_audit_log` | Histórico e trilha de eventos comerciais | `id (UUID)` | `request_id_idx`, `created_at_idx` | Admin AAL2 | INSERT via RPC / Edge Function | Append-only |
...
| `financial_entries` | Lançamentos de Contas a Receber/Pagar | `id (UUID)` | `due_date_idx`, `type_idx` | Admin AAL2 | Mutations via Edge Function | Status efetivo derivado em leitura |
| `financial_settlements` | Liquidações financeiras e baixas | `id (UUID)` | `entry_id_idx`, `settled_at_idx` | Admin AAL2 | Mutations via Edge Function | Suporte a estorno imutável (`reversed_at`) |
| `financial_audit_log` | Auditoria de baixas e estornos contábeis | `id (UUID)` | `settlement_id_idx`, `actor_idx` | Admin AAL2 | INSERT (Edge Function) | **Append-only**: DELETE e UPDATE revogados |
...
- **Políticas**:
  - `SELECT`: Apenas usuários com `admin_users` e `aal = 'aal2'` ou via URLs assinadas geradas no backend.
