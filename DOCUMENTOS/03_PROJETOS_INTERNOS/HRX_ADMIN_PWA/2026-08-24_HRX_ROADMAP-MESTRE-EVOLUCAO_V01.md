...
3. **Módulo Financeiro Consolidado**:
   ...
   - Exclusão de liquidações estornadas (`reversed_at IS NULL`) dos cálculos contábeis.
...
5. **Governança de PRs Históricas e Documentação**:
   ...
   - Arquivamento de relatórios canônicos em `DOCUMENTOS/03_PROJETOS_INTERNOS/HRX_ADMIN_PWA/`.
...
- **Critérios**:
  - Login com MFA e confirmação do claim `aal2` no JWT.
...
- **Objetivo**: Estruturar a matriz formal de governança de dados pessoais tratados na plataforma como proposta técnica a ser validada juridicamente.
- **Matriz de Tratamento (Proposta Técnica)**:
...
- **Requisitos**:
  ...
  - Webhook seguro em Edge Function para recepção de status de entrega (`sent`, `delivered`, `read`, `failed`).
...
|---|---|---|---|---|---|---|---|
| **F-00** | Fase 0 | Correção Documental de Fechamento e Rastreabilidade | P0 | Nenhuma | **CONCLUÍDO** | `main` (`44a5e49`) | `2026-08-24_HRX_RELATORIO-FINAL-MERGE-DEPLOY-HOMOLOGACAO_V01.md` |
...
1. Código implementado em branch dedicada, testado localmente com 100% de aprovação na suíte de testes (`npm run test:pwa` e `npx playwright test`).
