...
const projects: ProjectPanel[] = [
  {
    ...
    completed: ['Site institucional e identidade premium', 'Cadastro PF/PJ e consulta de CNPJ', 'Catálogo, pedidos, histórico e status', 'Consolidação de pedidos para compra', 'PWA, Web Push e deploy de produção'],
    pending: ['Concluir o ciclo operacional completo', 'Fechar integrações e regras fiscais pendentes', 'Executar revisão final web desktop e mobile', 'Consolidar documentação técnica e de entrega'],
  ...
  return <section className="admin-projects-shell" aria-labelledby="admin-projects-title">
    ...
    <div className="admin-projects-layout">
      <aside className="admin-projects-list" aria-label="Projetos">
        {projects.map((project) => <button key={project.id} type="button" className={selected.id === project.id ? 'is-active' : ''} onClick={() => setSelectedId(project.id)}>
