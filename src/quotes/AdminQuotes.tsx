...
export default function AdminQuotes() {
  ...
  return <section className={`admin-live-shell quote-commercial-shell${mobileDetail ? ' is-mobile-detail-open' : ''}`}>
    <section className="admin-exec-main">
      <header className="admin-exec-topbar quote-topbar"><div><span className="admin-section-kicker">COMERCIAL</span><h1>Orçamentos</h1><p>Crie, negocie e acompanhe propostas comerciais.</p></div><div><button className="quote-secondary" onClick={() => void load()} disabled={loading}>{loading ? 'Atualizando…' : 'Atualizar'}</button><button className="quote-primary" onClick={() => setNewOpen(true)}>{icons.plus}Novo orçamento</button></div></header>
      ...
      <div className="admin-workspace quote-workspace">
        <aside className="admin-queue quote-queue">
          ...
          <div className="admin-queue-list">{visible.map((request) => <button type="button" className={selectedId === request.id ? 'admin-lead is-active' : 'admin-lead'} key={request.id} onClick={() => openQuote(request)}><div className="admin-lead-top"><strong>{request.proposal_number}</strong><time>{new Date(request.draft.updated_at || request.created_at).toLocaleDateString('pt-BR')}</time></div><span>{request.company || request.name}</span><div className="admin-lead-bottom"><span className={`quote-status is-${request.draft.commercial_status}`}>{statusLabels[request.draft.commercial_status]}</span><b>{currency.format(Number(request.draft.final_amount || 0))}</b></div></button>)}{!loading && !visible.length && <div className="quote-empty">{icons.file}<strong>Nenhuma proposta encontrada</strong><span>Crie um orçamento usando um cliente real.</span></div>}</div>
        </aside>
...
function QuoteEditor({ request, clients, providers, pricingRules, session, onMutate, onError, onBack }: { request: Request; clients: Client[]; providers: ProviderRule[]; pricingRules: PricingRule[]; session: Session; onMutate: <T>(body: Record<string, unknown>, select?: string) => Promise<T>; onError: (value: string) => void; onBack: () => void }) {
  ...
  return <div className="quote-editor">
    <header className="quote-editor-header"><button className="quote-back" onClick={onBack}>{icons.back}</button><div><span>{request.proposal_number}</span><h2>{state.proposalTitle || 'Proposta sem título'}</h2><p>{request.company || request.name}</p></div><div className="quote-header-tools"><div className="quote-header-value"><span className={`quote-status is-${request.draft.commercial_status}`}>{statusLabels[request.draft.commercial_status]}</span><small>Valor total</small><strong>{currency.format(preview.finalAmount)}</strong></div><button className="quote-secondary" onClick={() => void onMutate({ action: 'duplicate_quote', requestId: request.id }, request.id)}>{icons.copy}Duplicar</button><button className="quote-secondary quote-more" aria-label="Abrir versões e envio" title="Versões e envio" onClick={() => setStep('send')}>•••</button></div></header>
    ...
    <div className="quote-editor-scroll">{isReadOnly && <div className="quote-readonly-banner" role="status"><strong>🔒 Proposta aprovada — somente leitura</strong><span>Duplique para criar uma nova negociação.</span></div>}<div className="quote-editor-main">
    ...
    {!isReadOnly && <footer className="quote-editor-footer"><div className={`quote-save-state is-${saveState}`}><span/>{saveState === 'saved' ? 'Salvo agora' : saveState === 'saving' ? 'Salvando…' : saveState === 'error' ? 'Falha ao salvar' : 'Alterações pendentes'}</div><div>{request.draft.commercial_status === 'draft' && request.draft.current_version === 0 && request.draft.status !== 'suspended' && <button className="quote-danger" onClick={() => setConfirmDelete(true)}>{icons.trash}Excluir rascunho</button>}<button className="quote-primary" disabled={saveState === 'saving'} onClick={() => void save()}>{saveState === 'saving' ? 'Salvando…' : 'Salvar'}</button></div></footer>}
