...
export default function AdminBootstrapAccess() {
  ...
  const valid = useMemo(() => (
    email.trim().length > 3 && code.trim().length >= 10 && password.length >= 8 && password === confirmPassword
  ...
  return (
    <>
      <button className="admin-bootstrap-launcher" type="button" onClick={() => setOpen(true)}>
        Ativar primeiro acesso
      </button>

      {open && (
        <div className="admin-bootstrap-overlay" role="presentation" onMouseDown={(event) => {
          ...
          <form className="admin-bootstrap-modal" onSubmit={activate} role="dialog" aria-modal="true" aria-labelledby="admin-bootstrap-title">
            <div className="admin-bootstrap-heading">
              <div>
                <span>HRX · PRIMEIRO ACESSO</span>
                <h2 id="admin-bootstrap-title">Ativar acesso</h2>
              </div>
              <button type="button" className="admin-bootstrap-close" onClick={close} aria-label="Fechar">×</button>
            </div>
